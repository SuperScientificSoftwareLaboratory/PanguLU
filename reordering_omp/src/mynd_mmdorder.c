...
void MMD_Order_line(graph_t *graph, reordering_int_t *reflect, reordering_int_t *reordernum, reordering_int_t task_id)
{
    ...
    for(reordering_int_t i = graph->nvtxs - 1;i >= 0;i--)
    {
        ...
        // {
        //     printf("task_id=%"PRIDX" reordernum=%"PRIDX" graph->label[i]=%"PRIDX" \n",task_id, reordernum,graph->label[i]);
        // }
        reflect[graph->label[i]] = --reordernum[task_id];

        // if( reflect[graph->label[i]] < 0 || reflect[graph->label[i]] != reordernum)
        //     printf("MMD_Order reordernum=%"PRIDX" graph->label[i]=%"PRIDX" \n", reordernum,graph->label[i]);
        // reordernum--;
    }
    // printf("end task_id=%"PRIDX" reordernum=%"PRIDX" graph->nvtxs=%"PRIDX" \n",task_id, reordernum[task_id],graph->nvtxs);
}

/***************************************************************************
*    mmdint ---- mult minimum degree initialisation
*    purpose -- this routine performs initialisation for the
     ...
     reordering_int_t maxint, reordering_int_t *ncsub)
{
    ...
    /* initialisation for the minimum degree algorithm. */
    *ncsub = 0;
